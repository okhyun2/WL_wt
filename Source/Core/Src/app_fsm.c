...
#define APP_FSM_TX_METER_PROXIMITY_GUARD_MS   (10u * 1000u)
#define APP_FSM_TX_SERVICE_MGMT_GAP_MS        (20u * 1000u)
#define APP_FSM_TX_DEVICE_SPREAD_MAX_MS       (3u * 1000u)
#else
#define APP_FSM_TX_SERVICE_SLOT_OFFSET_MS     (3u * 60000u)
...
static uint8_t App_FsmScheduleIsEarlierOrEqual(uint32_t lhsDateKey,
                                               ...
                                               uint32_t rhsDateKey,
                                               uint32_t rhsMsOfDay);
static AppStatus_t App_FsmTxScheduleApplyMeterProximityGuard(uint32_t *p_dueDateKey,
...
static AppStatus_t App_FsmNfcDebugProbePassThroughTail(const char *stage)
{
#if (APP_FSM_NFC_PT_TAIL_TEST_ENABLE == 1U)
    uint8_t dataBlock[4] = {0};
    uint8_t syncBlock[4] = {0};
    NFC_Result_t ret;

    ret = NFC_NTP53321_ReadBlock(&g_nfcTagHandle,
                                 APP_FSM_NFC_PT_TEST_BLOCK_DATA,
                                 dataBlock);
    if (ret != NFC_RESULT_OK)
    {
        APP_LOGW("FSM",
                 "trace nfc pt read fail stage=%s block=0x%04X ret=%d",
                 (stage != NULL) ? stage : "-",
                 (unsigned int)APP_FSM_NFC_PT_TEST_BLOCK_DATA,
                 (int)ret);
    ...
    {
        APP_LOGW("FSM",
                 "trace nfc pt sync-read fail stage=%s block=0x%04X ret=%d",
                 (stage != NULL) ? stage : "-",
                 (unsigned int)APP_FSM_NFC_PT_TEST_BLOCK_SYNC,
                 (int)ret);
        return APP_STATUS_OK;
    }

    APP_LOGI("FSM",
             "trace nfc pt tail stage=%s 203E=%02X %02X %02X %02X 203F=%02X %02X %02X %02X",
             (stage != NULL) ? stage : "-",
             (unsigned int)dataBlock[0],
             (unsigned int)dataBlock[1],
             (unsigned int)dataBlock[2],
             (unsigned int)dataBlock[3],
             (unsigned int)syncBlock[0],
...
static AppStatus_t App_FsmNfcVerifyPassThroughTailOnly(const char *stage)
{
    ...
    {
        APP_LOGW("FSM",
                 "trace nfc pt verify fail stage=%s block=0x%04X ret=%d",
                 (stage != NULL) ? stage : "-",
                 (unsigned int)APP_FSM_NFC_PT_TEST_BLOCK_DATA,
                 (int)ret);
        return APP_STATUS_OK;
    ...
    {
        APP_LOGW("FSM",
                 "trace nfc pt sync fail stage=%s block=0x%04X ret=%d",
                 (stage != NULL) ? stage : "-",
                 (unsigned int)APP_FSM_NFC_PT_TEST_BLOCK_SYNC,
                 (int)ret);
        return APP_STATUS_OK;
                                           ...
                                           uint8_t *p_status1)
{
    ...
    } while ((HAL_GetTick() - startTick) < APP_FSM_NFC_PT_WAIT_TIMEOUT_MS);
...
static AppStatus_t App_FsmNfcProcessWakeEvent(void)
{
    ...
    /* IRQ pending -> deferred ED event 반영 + wakeup 처리 */
...
#if (APP_FSM_NFC_PT_VERIFY_ONLY_ENABLE == 1U)
{
    ...
    APP_LOGI("FSM",
             "trace nfc pt-only mode active: wait sync/data-ready before read");

    if (App_FsmNfcWaitPtRxReady(&status0, &status1) != APP_STATUS_OK)
    {
        APP_LOGW("FSM",
                 "trace nfc pt-only skip read: not ready status0=0x%02X status1=0x%02X",
                 (unsigned int)status0,
                 (unsigned int)status1);
        return APP_STATUS_OK;
    }

    APP_RETURN_IF_FALSE(App_FsmNfcVerifyPassThroughTailOnly("ED_WAKE") == APP_STATUS_OK,
                        APP_STATUS_FATAL);
...
#if (APP_FSM_NFC_PT_TAIL_TEST_ENABLE == 1U)
    APP_LOGI("FSM", "trace nfc pt-tail test active: read 0x203E then 0x203F(release)");
    APP_RETURN_IF_FALSE(App_FsmNfcDebugProbePassThroughTail("ED_WAKE") == APP_STATUS_OK,
                        APP_STATUS_FATAL);
    return APP_STATUS_OK;
#endif
...
static AppStatus_t App_FsmScheduleAddMs(uint32_t baseDateKey,
                                        ...
                                        uint32_t *p_dueMsOfDay)
{
    ...
    status = App_FsmScheduleComposeDateTime(baseDateKey, baseMsOfDay, &dueTime);
    APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);

    nextMs = baseMsOfDay + addMs;
    while (nextMs >= (24u * 3600000u))
    {
        nextMs -= (24u * 3600000u);
        App_FsmMeterScheduleAddOneDay(&dueTime);
...
static AppStatus_t App_FsmTxScheduleCalcPeriodMs(uint8_t periodHours,
                                                  uint32_t debugPeriodMs,
                                                  uint32_t *p_periodMs)
{
    APP_RETURN_IF_FALSE(p_periodMs != NULL, APP_STATUS_INVALID_PARAM);
    APP_RETURN_IF_FALSE(periodHours > 0u, APP_STATUS_INVALID_PARAM);

    if (debugPeriodMs != 0u)
    {
        *p_periodMs = debugPeriodMs;
    }
    else
    {
        *p_periodMs = (uint32_t)periodHours * 3600000u;
    }
    return APP_STATUS_OK;
}
...
{
    AppStatus_t status;
    uint8_t meterPeriodHours;
    uint32_t meterPeriodMs;
    uint32_t guardMs;
    uint32_t offsetWithinMeterMs;
    uint32_t shiftMs;

    APP_RETURN_IF_FALSE((p_dueDateKey != NULL) && (p_dueMsOfDay != NULL), APP_STATUS_INVALID_PARAM);

    status = App_FsmMeterScheduleLoadPeriod(&meterPeriodHours);
    APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
    ...
    guardMs = APP_FSM_TX_METER_PROXIMITY_GUARD_MS;
    if (guardMs >= meterPeriodMs)
    {
        guardMs = meterPeriodMs / 2u;
    }
    if (guardMs == 0u)
    {
        return APP_STATUS_OK;
    ...
    offsetWithinMeterMs = (*p_dueMsOfDay % meterPeriodMs);
    if (offsetWithinMeterMs < guardMs)
    {
        shiftMs = guardMs - offsetWithinMeterMs;
        status = App_FsmScheduleAddMs(*p_dueDateKey, *p_dueMsOfDay, shiftMs, p_dueDateKey, p_dueMsOfDay);
        APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
        return APP_STATUS_OK;
    }

    shiftMs = meterPeriodMs - offsetWithinMeterMs;
    if (shiftMs < guardMs)
    {
        status = App_FsmScheduleAddMs(*p_dueDateKey, *p_dueMsOfDay, shiftMs + guardMs, p_dueDateKey, p_dueMsOfDay);
        APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
    }
...
static AppStatus_t App_FsmTxSchedulePlanBootstrap(const AppDateTime_t *p_now,
                                                  const AppFsmTxScheduleContext_t *p_schedule,
                                                  uint32_t debugPeriodMs,
                                                  uint32_t *p_dueDateKey,
                                                  uint32_t *p_dueMsOfDay,
                                                  uint32_t *p_jitterMs)
{
    AppStatus_t status;
    uint32_t periodMs;
    uint32_t baseDateKey;
    uint32_t baseMsOfDay;
    uint32_t jitterMs;
    uint32_t totalOffsetMs;

    APP_RETURN_IF_FALSE((p_now != NULL) && (p_schedule != NULL) && (p_dueDateKey != NULL) && (p_dueMsOfDay != NULL), APP_STATUS_INVALID_PARAM);

    status = App_FsmTxScheduleCalcPeriodMs(p_schedule->periodHours, debugPeriodMs, &periodMs);
    APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
    ...
    jitterMs = App_FsmTxScheduleCalcJitterMs(baseDateKey, baseMsOfDay, p_schedule->periodHours);
    totalOffsetMs = p_schedule->baseSlotOffsetMs + p_schedule->fixedOffsetMs + jitterMs;
    ...
    status = App_FsmTxScheduleApplyMeterProximityGuard(p_dueDateKey, p_dueMsOfDay);
    APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
...
static AppStatus_t App_FsmTxScheduleAdvanceFromDue(AppFsmTxScheduleContext_t *p_schedule,
                                                   uint32_t baseDateKey,
                                                   uint32_t baseMsOfDay,
                                                   uint32_t debugPeriodMs,
                                                   uint32_t *p_dueDateKey,
                                                   ...
                                                   uint32_t *p_jitterMs)
{
    ...
    APP_RETURN_IF_FALSE((p_schedule != NULL) && (p_dueDateKey != NULL) && (p_dueMsOfDay != NULL), APP_STATUS_INVALID_PARAM);
...
static AppStatus_t App_FsmMgmtTxScheduleLoadPeriod(uint8_t *p_periodHours)
{
    ...
    *p_periodHours = options.managementReportingPeriodHours;
    return APP_STATUS_OK;
}
                                                            ...
                                                            uint32_t debugPeriodMs)
{
    ...
    APP_LOGI("FSM", "%s enabled period=%uh mode=period-offset next=%lu %02lu:%02lu:%02lu base_offset=%lu spread=%lu jitter=%lu",
             ...
             (unsigned long)p_schedule->baseSlotOffsetMs,
...
static AppStatus_t App_FsmTxScheduleEnsureInitialized(void)
{
    ...
    status = App_FsmMgmtTxScheduleApplyServiceGapGuard();
    APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
    return APP_STATUS_OK;
}
...
static AppStatus_t App_FsmUsimHoldActivate(uint8_t phase)
{
    ...
    holdSeconds = holdHours * 3600u;
    g_appFsmUsimHold.remainingWakeupSeconds = holdSeconds;

    wakeupChunk = (g_appFsmUsimHold.remainingWakeupSeconds > APP_FSM_USIM_HOLD_WUT_MAX_SECONDS) ?
                  APP_FSM_USIM_HOLD_WUT_MAX_SECONDS : g_appFsmUsimHold.remainingWakeupSeconds;
    status = App_SystemRequestShortRtcWakeup(wakeupChunk);
    APP_RETURN_IF_FALSE(status == APP_STATUS_OK, status);
...
static AppStatus_t App_FsmUsimHoldGateNbiotWake(uint8_t *p_blocked)
{
    ...
    *p_blocked = APP_FALSE;
    ...
    if (g_appFsmUsimHold.rtcValidMode == APP_TRUE)
    {
        ...
        {
            *p_blocked = APP_TRUE;
            APP_LOGW("FSM", "[[UsimHold]] phase=%s hold active but RTC lost -> keep meter/tx blocked",
    ...
    if (g_appFsmUsimHold.remainingWakeupSeconds > 0u)
    {
        wakeupChunk = (g_appFsmUsimHold.remainingWakeupSeconds > APP_FSM_USIM_HOLD_WUT_MAX_SECONDS) ?
                      APP_FSM_USIM_HOLD_WUT_MAX_SECONDS : g_appFsmUsimHold.remainingWakeupSeconds;
        status = App_SystemRequestShortRtcWakeup(wakeupChunk);
...
AppStatus_t App_FsmGetNextTxDueTime(AppDateTime_t *p_dueTime)
{
    ...
    if (serviceStatus == APP_STATUS_OK)
    {
                                          ...
                                          nowMsOfDay) == APP_TRUE)
        {
            ...
            status = App_FsmTxScheduleAdvanceFromDue(&g_appFsmTxSchedule,
                                                     staleDateKey,
                                                     staleMsOfDay,
#ifdef APP_DEBUG_TX_PERIOD_MS
                                                     APP_DEBUG_TX_PERIOD_MS,
