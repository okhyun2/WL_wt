...
static AppStatus_t App_FsmNfcWriteIndicateResponse(uint8_t prefix, uint8_t addr, uint8_t blockLen, uint8_t suffix)
{
    ...
    APP_LOGI("FSM", "trace nfc indicate raw %02X %02X %02X %02X",
             ...
             (unsigned int)indBuf[1],
             (unsigned int)indBuf[2],
             (unsigned int)indBuf[3]);
...
static AppStatus_t App_FsmNfcHandleIndicatedCommand(AppNfcSeoulProcessResult_t *p_seoulResult)
{
    ...
    if (byteCount >= 4u)
    {
        ...
        /* CMD: METER */
        if (((raw[0] == NFC_METER_CMD_PREFIX) &&
             (raw[1] == APP_NFC_SEOUL_CMD_REQ_GROUP)) &&
            ((raw[2] == APP_NFC_SEOUL_CMD_STOR_REQ) ||
             (raw[2] == APP_NFC_SEOUL_CMD_MTR_REQ) ||
             (raw[2] == APP_NFC_SEOUL_CMD_AMI_REQ) ||
        ...
        if ((raw[0] == NFC_AUTH_CMD_PREFIX) &&
            ((raw[1] == NFC_AUTH_CMD_CONNECT) ||
             (raw[1] == NFC_AUTH_CMD_CHALLENGE_READY) ||
             (raw[1] == NFC_AUTH_CMD_RESPONSE) ||
...
static void App_FsmNfcTraceUcmdDispatch(void)
{
    ...
    {
        APP_LOGW("FSM",
                 "trace ucmd peek cmd read fail blk=0x%04X ret=%d",
    ...
    APP_LOGI("FSM",
             "trace ucmd peek cmd=%02X %02X %02X %02X auth=%s sess=%u",
             (unsigned int)cmdWord[0],
             (unsigned int)cmdWord[1],
             (unsigned int)cmdWord[2],
             (unsigned int)cmdWord[3],
             App_FsmNfcGetAuthStateName(g_nfcAuthHandle.state),
             (unsigned int)(g_nfcAuthHandle.session.active ? 1u : 0u));

    if (cmdWord[0] != NFC_APP_CTRL_CMD_CLASS)
    {
        return;
    }
    ...
    APP_LOGI("FSM",
             "trace appctrl payload0=%02X %02X %02X %02X",
